"""Command line interface for Model Organism Gene Transform."""
import logging
...
def callback(version: bool = typer.Option(False, "--version", is_eager=True),
    ...
    """Model Organism Gene Transform CLI."""
    if version:
        from model_organism_gene_transform import __version__
        typer.echo(f"Model Organism Gene Transform version: {__version__}")
        raise typer.Exit()


@app.command()
def download(force: bool = typer.Option(False, help="Force download of data, even if it exists")):
    """Download data for Model Organism Gene Transform."""
    typer.echo("Downloading data for Model Organism Gene Transform...")
...
def transform(
    ...
    """Run the Koza transform for Model Organism Gene Transform."""
    typer.echo("Transforming data for Model Organism Gene Transform...")
